...
All DB timestamp columns are timestamptz; the dev database default timezone is Australia/Sydney (display-only convenience for SQL views). All frontend date display goes through formatDateAU/formatDateShortAU/formatDateTimeAU in the quotecraft lib/format module (Intl en-AU, Australia/Sydney) — never date-fns format or toLocaleDateString, which render in device-local time.
**Why:** date-fns format silently uses the viewer's timezone; and SQL that used to_char/extract on timestamps relied on the session timezone, which differs per environment. Month/year filters in quote + email routes now say AT TIME ZONE 'Australia/Sydney' explicitly.
